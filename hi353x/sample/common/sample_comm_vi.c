    ...
    SAMPLE_VI_MODE_E enViMode, SAMPLE_VI_CHN_SET_E enViChnSet)
{
	...
	s32Ret = HI_MPI_VI_GetChnAttr(ViChn, &attr);
	if (s32Ret != HI_SUCCESS) {
		SAMPLE_PRT("failed with %x\n", s32Ret);
		return HI_FAILURE;
	}
...
*****************************************************************************/
HI_S32 SAMPLE_COMM_VI_Start2(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm,
		int dev_result[8],
		int ch_result[16], int subch_result[16])
{
    VI_DEV ViDev;
...
for(i=0; i < stViParam.s32ViChnCnt; i++) {
	...
	if (HI_TRUE == SAMPLE_COMM_VI_IsHd(enViMode))
	{
		...
		if (HI_SUCCESS != s32Ret) {
	SAMPLE_PRT("%d,%d,%d,%d, %d-%d\n",
			stCapRect.s32X, stCapRect.s32Y,
			stCapRect.u32Width, stCapRect.u32Height,
			stMainTargetSize.u32Width, stMainTargetSize.u32Height);
			SAMPLE_PRT("SAMPLE_COMM_VI_StartChn (Sub_Chn-%d) failed!\n", ViChn_Sub);
			subch_result[ViChn] = -1;
		}
		else {
			subch_result[ViChn] = 1;
		}
