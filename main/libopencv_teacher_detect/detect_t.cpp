...
HI_S32 TeacherDetecting::hi_luv_method( std::vector<Mat> img, std::vector<Mat> bg, Mat &dst)
{
	...
	ftime(&pre_last);
	/*timeb pre,cur,cur_last;
	ftime(&pre);*/

	Mat img0 = img[0]; Mat img1 = img[1]; Mat img2 = img[2];
	Mat bg0 = bg[0]; Mat bg1 = bg[1]; Mat bg2 = bg[2];
	Mat yuv0,yuv1,yuv2;
	Mat luv0,luv1,luv2;
	Mat src = img0;
	...
	//跨度字节对齐;
	int width = src.cols; int src_addr = (int)src.data;//?????这个不知道理解的正不正确;
	int stride_t = {8-((width+(src_addr%8))%8)%8+(src_addr%8)+width};
	int stride = src.cols;
	if(stride<stride_t)
		stSrc_img0.stSrcMem.u32Stride = (stride_t/8)*8+(stride_t%8);
	else
		stSrc_img0.stSrcMem.u32Stride = (stride/8)*8+(stride%8);
	...
	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_img0.stSrcMem.u32PhyAddr,&pVirtSrc_img0,
		"User",HI_NULL,stSrc_img0.u32Height*stSrc_img0.stSrcMem.u32Stride);

	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_img1.stSrcMem.u32PhyAddr,&pVirtSrc_img1,
		"User",HI_NULL,stSrc_img1.u32Height*stSrc_img1.stSrcMem.u32Stride);

	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_img2.stSrcMem.u32PhyAddr,&pVirtSrc_img2,
		"User",HI_NULL,stSrc_img2.u32Height*stSrc_img2.stSrcMem.u32Stride);

	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_bg0.stSrcMem.u32PhyAddr,&pVirtSrc_bg0,
		"User",HI_NULL,stSrc_bg0.u32Height*stSrc_bg0.stSrcMem.u32Stride);

	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_bg1.stSrcMem.u32PhyAddr,&pVirtSrc_bg1,
		"User",HI_NULL,stSrc_bg1.u32Height*stSrc_bg1.stSrcMem.u32Stride);

	s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stSrc_bg2.stSrcMem.u32PhyAddr,&pVirtSrc_bg2,
	...
	for(int i = 0;i<img0.rows;i++)
	{
		...
		p2 = (uchar*)pVirtSrc_img0 + i * stSrc_img0.stSrcMem.u32Stride;
		memcpy(p2,p1,img0.cols);

		p1 = img1.data + i * img1.cols;
		p2 = (uchar*)pVirtSrc_img1 + i * stSrc_img1.stSrcMem.u32Stride;
		memcpy(p2,p1,img1.cols);

		p1 = img2.data + i * img2.cols;
		p2 = (uchar*)pVirtSrc_img2 + i * stSrc_img2.stSrcMem.u32Stride;
	...
	IVE_SUB_OUT_FMT_E enOutFmt; enOutFmt = IVE_SUB_OUT_FMT_ABS;
	...
	s32Ret = HI_MPI_IVE_SUB(&IveHandle,&stSrc_img0,&stSrc_bg0,&stDst,enOutFmt,bInstant);
	IVE_SRC_INFO_S stThInfo;
	memcpy(&stThInfo, &stSrc_img0, sizeof(IVE_SRC_INFO_S));
	stThInfo.stSrcMem.u32PhyAddr = stDst.u32PhyAddr;
	s32Ret = HI_MPI_IVE_THRESH(&IveHandle,&stThInfo,&stDst_L,&pstThreshCtrl,bInstant);

	HI_MPI_IVE_SUB(&IveHandle,&stSrc_img1,&stSrc_bg1,&stDst,enOutFmt,bInstant);
	pstThreshCtrl.u32Thresh = 22;
	s32Ret = HI_MPI_IVE_THRESH(&IveHandle,&stThInfo,&stDst_U,&pstThreshCtrl,bInstant);

	HI_MPI_IVE_SUB(&IveHandle,&stSrc_img2,&stSrc_bg2,&stDst,enOutFmt,bInstant);
	pstThreshCtrl.u32Thresh = 22;
	s32Ret = HI_MPI_IVE_THRESH(&IveHandle,&stThInfo,&stDst_V,&pstThreshCtrl,bInstant);
...
HI_S32 TeacherDetecting::hi_dilate( Mat src, Mat &dst)
{
	...
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stSrc,&stDst,&pstDilateCtrl,bInstant);
	if(s32Ret != HI_SUCCESS)
	{
		HI_MPI_SYS_MmzFree(stDst.u32PhyAddr, pVirtDst);
...
*/
	...
	s32Ret = HI_MPI_IVE_ERODE(&IveHandle,&stDilate1Info,&stDst,&pstErodeCtrl,bInstant);
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stDilate1Info,&stDst,&pstDilateCtrl,bInstant);
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stDilate1Info,&stDst,&pstDilateCtrl,bInstant);
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stDilate1Info,&stDst,&pstDilateCtrl,bInstant);
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stDilate1Info,&stDst,&pstDilateCtrl,bInstant);
	s32Ret = HI_MPI_IVE_DILATE(&IveHandle,&stDilate1Info,&stDst,&pstDilateCtrl,bInstant);
...
std::vector < Rect > TeacherDetecting::refineSegments2(Mat img, Mat & mask,
						       Mat & dst,double interval,double marea,double mrect_area)
{
	vector < Rect > rect;
	int niters = 2;
...
//YUV算法获取矩形框序列;
void TeacherDetecting::luv_method(const Mat &img ,std::vector<Mat> img_t_vec)
{
	Mat luv_m,luv_m_temp,fgimg;//背景减除;
	luv_m.create(Size(img.cols, img.rows), CV_8UC1);
	luv_m.setTo(0);
...
//*****************************帧差法*************************************
void TeacherDetecting::frame_difference_method (Mat image,std::vector<Rect> &masked_frame_rect,Mat Y)
{
	timeb pre,cur;
	ftime(&pre);
	Mat silh;silh.create(Size(image.cols,image.rows),CV_8UC1);
	...
	std::vector<Rect> masked_rect_vector_t;
	Mat dst_masked;
	masked_rect_vector_t = refineSegments2(image,silh,dst_masked,frame_s.interval,frame_s.minarea,frame_s.minrect);
	for(int i = 0;i<masked_rect_vector_t.size();i++)
	{
		Rect t = masked_rect_vector_t[i];
		masked_frame_rect.push_back(t);		
